...
def save_to_file():
        ...
        # --- 平滑导数计算 ---
        s_smooth = savgol_filter(s_vals, window_length=9, polyorder=3)
        v_vals = np.gradient(s_smooth, t_vals)
        v_smooth = savgol_filter(v_vals, window_length=9, polyorder=3)
        a_vals = np.gradient(v_smooth, t_vals)
        a_smooth = savgol_filter(a_vals, window_length=9, polyorder=3)
...
if __name__ == "__main__":
    ...
    def recompute_curves():
        ...
        # 更新图像
        line_v.set_data(t_vals, v_smooth)
        line_a.set_data(t_vals, a_smooth)
        for ax in [ax_v, ax_a]:
            ax.relim()
            ax.autoscale_view()
    ...
    def on_motion(event):
        ...
        s_vals = np.interp(t_vals, drag_t, drag_s)
