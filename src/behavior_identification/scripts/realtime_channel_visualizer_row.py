#!/usr/bin/env python3
"""Self-contained one-row expected-channel monitor.

Panels: CARLA real scene | ST speed channel | SL straight-road channel |
AT acceleration channel. The node waits for the fixed reckless-driving
vehicle, keeps the GUI responsive while waiting, and starts GIF encoding only
after the first complete rendered frame is available.
"""
...
class CarlaWindowCapture:
    """Capture the CARLA game window, with a configured rectangle fallback."""
...
class GifWriter:
    """Encode an RGB frame stream and atomically publish a valid GIF."""
    ...
    def write(self, frame: np.ndarray) -> bool:
        ...
        if self.process.poll() is not None:
            rospy.logerr_throttle(
                2.0, "GIF encoder exited early with code %s", self.process.returncode
            )
            return False
        ...
        if rgb.shape != (self.height, self.width, 3):
            rospy.logerr_throttle(
                2.0,
                "Skip GIF frame with shape %s; encoder expects (%d, %d, 3)",
                rgb.shape,
                self.height,
                self.width,
            )
    ...
    def close(self) -> None:
        ...
        try:
            ...
            rospy.logwarn("GIF encoder did not finish in 20 s; terminating it.")
        ...
        valid_file = (
            ...
            and self.temp_path.stat().st_size > 0
        )
        if valid_file:
        ...
        try:
            ...
            if self.temp_path.exists():
                self.temp_path.unlink()
        rospy.logerr(
            "GIF was not saved: encoder_code=%s frames=%d error=%s",
            return_code,
            self.frame_count,
            stderr_text.strip() or "no ffmpeg error text",
        )
...
class ExpectedChannelMonitor:
    def __init__(self) -> None:
        ...
        self.figure_width = max(int(rospy.get_param("~gif_width", 1920)), 960)
        self.figure_height = max(int(rospy.get_param("~gif_height", 540)), 320)
        ...
        rospy.Subscriber(
            "/carla/vehicle_config", VehicleConfig, self.vehicle_config_callback,
            queue_size=50,
        )
        rospy.Subscriber(
            "/veh_state_sequences", VehStateSequenceArray,
            self.vehicle_state_callback, queue_size=1,
        )
        rospy.Subscriber(
            "/scenario_status", String, self.scenario_status_callback,
            queue_size=10,
        )
        rospy.on_shutdown(self.close)

        rospy.loginfo(
            "Expected-channel monitor started: show_window=%d save_gif=%d DISPLAY=%s",
    ...
    def configure_sl_mode_no_lock(self, msg: VehicleConfig) -> None:
        ...
        explicit_turn = any(
            ...
            for token in (
                "left_turn", "right_turn", "turn_left", "turn_right", "左转", "右转"
            )
        )
    ...
    def vehicle_state_callback(self, msg: VehStateSequenceArray) -> None:
        ...
        if first_sample:
            rospy.loginfo(
                "Target appeared in /veh_state_sequences; live drawing starts now."
            )
        rospy.loginfo_throttle(
            2.0,
            "Channel input %s(%d): speed=%.3f accel=%.3f poses=%d",
            role_name,
            target_id,
            speed,
            acceleration,
    ...
    def draw_st(self, times: np.ndarray, speed: np.ndarray) -> None:
        self.ax_st.clear()
        ...
        self.ax_st.plot(times[valid], speed[valid], linewidth=2.0, label="Actual speed")
        too_slow = valid & (speed < self.min_speed)
        too_fast = valid & (speed > self.max_speed)
        if np.any(too_slow):
            self.ax_st.scatter(
                times[too_slow], speed[too_slow], marker="v", s=24,
                label="Too slow",
            )
        if np.any(too_fast):
            self.ax_st.scatter(
                times[too_fast], speed[too_fast], marker="^", s=24,
                label="Too fast",
            )
    ...
    def draw_sl(
        ...
        self.ax_sl.plot(
            times[valid], lateral[valid], linewidth=2.0,
            label="Actual lateral offset",
        )
        abnormal = valid & (np.abs(lateral) > self.sl_limit)
        if np.any(abnormal):
            self.ax_sl.scatter(
                times[abnormal], lateral[abnormal], marker="x", s=28,
    ...
    def draw_at(self, times: np.ndarray, acceleration: np.ndarray) -> None:
        self.ax_at.clear()
        ...
        self.ax_at.plot(
            times[valid], acceleration[valid], linewidth=2.0,
            label="Actual acceleration",
        )
        abnormal = valid & (np.abs(acceleration) > self.max_accel)
        if np.any(abnormal):
            self.ax_at.scatter(
                times[abnormal], acceleration[abnormal], marker="x", s=28,
                label="Outside channel",
            )

        current = float(acceleration[valid][-1])
    ...
    def start_gif_from_frame(self, frame: np.ndarray) -> None:
        ...
        try:
            ...
            rospy.loginfo(
                "GIF encoder opened after first rendered frame: %s (%dx%d)",
                output_path,
                width,
                height,
            )
    ...
    def draw(self) -> None:
        ...
        try:
            self.draw_scene()
            self.draw_st(data["time"], data["speed"])
            self.draw_sl(
                data["time"],
                data["lateral"],
    ...
    def close(self) -> None:
        ...
        if writer is not None:
            rospy.loginfo("Finalizing GIF with %d frames...", writer.frame_count)
            writer.close()
        elif self.save_gif:
            rospy.logwarn(
                "No GIF was created because no complete channel frame was rendered."
            )
