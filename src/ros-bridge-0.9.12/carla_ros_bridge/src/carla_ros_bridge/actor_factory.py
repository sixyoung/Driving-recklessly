...
class ActorFactory(object):
    ...
    def _spawn_carla_actor(self, req):
        ...
        if isinstance(carla_actor, carla.Vehicle):
            carla_actor.apply_control(carla.VehicleControl(brake=1.0))
    ...
    def _create_object(self, uid, type_id, name, attach_to, spawn_pose, carla_actor=None):
        ...
        if type_id == TFSensor.get_blueprint_name():
            ...
            if (carla_actor.attributes.get('role_name').startswith("hero") or 
                carla_actor.attributes.get('role_name') in self.node.parameters['ego_vehicle']['role_name']):
                actor = EgoVehicle(
