...
class DebugHelper(object):
    ...
    def on_marker(self, marker_array):
        ...
        for marker in marker_array.markers:
            ...
            if marker.type == Marker.POINTS:
                ...
                self.draw_text(marker, lifetime, color)
            else:
                self.node.logwarn(
    ...
    def draw_text(self, marker, lifetime, color):
        """
        draw text from ros marker
        """
        location = carla.Location(x=marker.pose.position.x, y=-marker.pose.position.y,
                                  z=marker.pose.position.z)
        text = marker.text
        # Optional: marker.scale.z is often used for text size in rviz, maybe use if passed.
        # CARLA doesn't easily scale draw_string size dynamically without draw_on_top or similar, 
        # but let's just use it natively.
        self.node.loginfo("Draw Text '{}' at {} (color: {}, lifetime: {})".format(
            text, location, color, lifetime))
        self.debug.draw_string(location, text, draw_shadow=True, color=color, life_time=lifetime)
