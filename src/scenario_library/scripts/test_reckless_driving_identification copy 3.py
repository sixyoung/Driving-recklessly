    ...
    with open(LOG_FILE, "a", newline="", encoding="utf-8") as f:
        ...
        if new_file:
            writer.writerow([
                ...
                pad("scenario_file"),
                pad("random_role"),
                pad("detection_result (behavior/vehicle/time)")
            ])

        writer.writerow([
            pad(datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            pad(f"{round_id}/{run_id}"),
            pad(scenario_class),
            pad(scenario_file),
            pad(random_role),
            pad(detection_msg)
...
def run_single_scenario(scn_file, round_id, run_id):
    ...
    if detection_msg is None:
        detection_msg = "no_detection"

    print(f"🔍 Detection Result from ROS: {detection_msg}")

    # 写日志
    scenario_class = find_scenario_class(scn_file)
    write_log(round_id, run_id, scn_file, scenario_class, random_role, detection_msg)
