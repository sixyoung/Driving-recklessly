...
ReferenceLine::ReferenceLine(CubicSpline2D *csp, const std::vector<ReferencePoint> &reference_points,
    ...
    : csp_(csp), reference_points_(reference_points), accumulated_s_(accumulated_s), reference_path_(reference_path), Config_(&cfg)
{
}

ReferencePoint ReferenceLine::GetReferencePoint(const double s) const {
  if (reference_points_.empty()) {
    throw std::runtime_error("ReferenceLine has no reference points.");
  }

  // s 小于起点
  if (s <= reference_points_.front().accumulated_s_) {
    return reference_points_.front();
  }
  // s 大于终点
  if (s >= reference_points_.back().accumulated_s_) {
    return reference_points_.back();
  }

  // 二分查找第一个 >= s 的点
  auto comp = [](const ReferencePoint &point, const double &s_val) {
    return point.accumulated_s_ < s_val;
  };
  auto it_upper = std::lower_bound(reference_points_.begin(),
                                   reference_points_.end(), s, comp);

  auto it_lower = it_upper - 1; // 前一个点

  const double s0 = it_lower->accumulated_s_;
  const double s1 = it_upper->accumulated_s_;
  const double ratio = (s - s0) / (s1 - s0);

  ReferencePoint p;
  p.x_ = it_lower->x_ + ratio * (it_upper->x_ - it_lower->x_);
  p.y_ = it_lower->y_ + ratio * (it_upper->y_ - it_lower->y_);
  p.heading_ = math::slerp(it_lower->heading_, it_lower->accumulated_s_,
                          it_upper->heading_, it_upper->accumulated_s_, s);
  p.kappa_   = it_lower->kappa_   + ratio * (it_upper->kappa_   - it_lower->kappa_);
  p.dkappa_  = it_lower->dkappa_  + ratio * (it_upper->dkappa_  - it_lower->dkappa_);
  p.accumulated_s_ = s;
...
bool ReferenceLine::XYToSL(const Vec2d &xy_point, SLPoint *const sl_point) const
{
  ...
  // double rs = reference_points_[index].s();                                                  //角度
  // double l = aux->cartesian_to_frenet(initpoint, nearest, rtheta);                                     // l
  // double s = aux->find_s(initpoint, nearest, rtheta, rs); // s

  auto sl_ = PathMatcher::GetPathFrenetCoordinate(reference_points_, xy_point.x(), xy_point.y());
  sl_point->set_s(sl_.first);
  sl_point->set_l(sl_.second);
  return true;
}
