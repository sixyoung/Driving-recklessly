...
DpPolyPathOptimizer::DpPolyPathOptimizer(const std::vector<const Obstacle *> &obstacles,
    ...
    : obstacles_(obstacles), vehicle_pos_(vehicle_pos),Config_(&cfg) //传入障碍物和车的里程计
{
}
DpPolyPathOptimizer::DpPolyPathOptimizer(const Param_Configs& cfg):Config_(&cfg)
{
}
...
{
  DpRoadGraph dp_road_graph(speed_data, reference_line, vehicle_pos_, *Config_); //构造DP路网图
  dp_road_graph.SetWaypointSampler(new WaypointSampler(*Config_));
  ...
  {
    ROS_INFO("[%s]:Failed to find tunnel in road graph!", Config_->role_name_.c_str());
    return false;
  }
