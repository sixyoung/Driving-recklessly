...
TrajectoryCost::TrajectoryCost(const VehicleConfig vehicle_config, const ReferenceLine &reference_line,
                               ...
                               const SpeedData &heuristic_speed_data, const SLPoint &init_sl_point, const Param_Configs &cfg)
...

{
  ...
  */
  ...
  for (const auto *ptr_obstacle : obstacles)
  {
    ...
    bool is_bycycle_or_pedestrian = (sl_boundary.obstacle_type == 4 || sl_boundary.obstacle_type == 6);
  ...
  if(Config_-> role_name_ == "hero0" && 0){
    planner::FileExporter::ExportTrajectoryCostObstacles(
        static_obstacle_sl_boundaries_,
        dynamic_obstacle_boxes_,
        "/home/bob/文档/备份/demo05/src/test/txt/trajectory_cost_obstacles/trajectory_cost_obstacles.txt",
        Config_->frame_id,
        true,   // append_mode
        true,   // split_mode
        300,     // 每300帧一个文件
                                                 ...
                                                 const uint32_t total_level)
{
  ...
  {
    const double l0 = Config_->path_l_cost_param_l0; //1.5
    const double b = Config_->path_l_cost_param_b;   //0.4
    const double k = Config_->path_l_cost_param_k;   //1.5
  ...
  for (double curve_s = 0.0; curve_s < (end_s - start_s); curve_s += Config_->path_resolution)
  {
    ...
    {
      ROS_WARN("[%s]:🚧 IsOffRoad triggered! s=%.3f (global_s=%.3f), l=%.3f, dl=%.6f",
              Config_->role_name_.c_str(),curve_s, curve_s + start_s, l, dl);
...
{
  static constexpr double kIgnoreDistance = 20.0;
  ...
  // ROS_WARN("IsOffRoad check: ref_s=%.3f l=%.3f dl=%.3f | left_bound=%.3f right_bound=%.3f rear_y=%.3f front_y=%.3f",
  //         ref_s, l, dl, left_bound, right_bound, rear_center.y(), front_center.y());
...
{
  ComparableCost obstacle_cost;
  if (obs_sl_boundary.start_l_ * obs_sl_boundary.end_l_ <= 0.0 || 1)
  {
    return obstacle_cost;
  }
  ...
  */
  // std::cout << "adc_s: " << adc_s << "; adc_l: " << adc_l << "; adc_left_l: " << adc_left_l
  //       << "; adc_right_l: " << adc_right_l << "; adc_front_s: " << adc_front_s << "; adc_end_s: " << adc_end_s
  //       << "; obs_sl_boundary.end_l_: " << obs_sl_boundary.end_l_ << "; obs_sl_boundary.start_l_: " << obs_sl_boundary.start_l_ << "; delta_l = " << delta_l << "\n";

  static constexpr double kSafeDistance = 0.6;
  if (delta_l < kSafeDistance)
  {
    //  侧方向距离造成的cost
    obstacle_cost.safety_cost += Config_->obstacle_collision_cost * math::Sigmoid(Config_->obstacle_collision_distance - delta_l);
  }
...
ComparableCost TrajectoryCost::GetCostBetweenObsBoxes(const Box2d &ego_box, const Box2d &obstacle_box) const
{
  ...
  // A. 计算碰撞cost
  obstacle_cost.safety_cost += Config_->obstacle_collision_cost * math::Sigmoid(Config_->obstacle_collision_distance - distance);
  obstacle_cost.safety_cost += 20.0 * math::Sigmoid(Config_->obstacle_risk_distance - distance);
  return obstacle_cost;
}
...
Box2d TrajectoryCost::GetBoxFromSLPoint(const SLPoint &sl, const double dl) const
{
  ...
  const double theta = math::NormalizeAngle(delta_theta + reference_point.heading());
