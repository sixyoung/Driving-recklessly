                                          ...
                                          std::vector<std::vector<SLPoint>> *const points)
{
  ...
  //纵向s: 以8秒的窗口来撒点，当init_point_v * 8小于50米时，以50米的范围撒点
  const double kMinSampleDistance = Config_->PathLength; // 前探采样距离50
  // init_point.DiscretizedPath_.point.front().v:轨迹第一个点的速度
  // 确定要规划的纵向距离，8秒*init_v或PathLength米
  const double total_length = std::fmin(init_sl_point_.s + std::fmax(init_point.v * 8.0, kMinSampleDistance), reference_line_->Length());
  const double half_adc_width = vehicle_config_.vehicle_width / 2.0; //自主车的半宽度
  //每个level的撒点数，即每一纵列撒点数，这里是选9个， navigation_mode是高速的
  ...
  const double level_distance =
      math::Clamp(init_point.v * kSamplePointLookForwardTime, Config_->step_length_min, Config_->step_length_max);
  // 初始s
  double accumulated_s = init_sl_point_.s; //先等于起点
  double prev_s = accumulated_s;

  static constexpr size_t kNumLevel = 3; //固定level的层数，为3层
  for (size_t ii = 0; ii < kNumLevel && accumulated_s < total_length; ++ii)
  {
    ...
    const double kChangeLaneDeltaL = math::Clamp(level_distance * (std::fabs(init_frenet_frame_point_.d) + delta_dl), 1.2, 3.5);
    ...
    {    
      // ROS_INFO("[%s]:IsChangeLanePath_1.", Config_->role_name_.c_str());
    ...
    else // 不是换道情况下，目前是这个
    {
      ...
      math::uniform_slice(sample_right_boundary, sample_left_boundary,
