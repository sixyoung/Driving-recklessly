...
PathTimeHeuristicOptimizer::PathTimeHeuristicOptimizer(const StGraphData &st_graph_data,
    ...
    : st_graph_data_(st_graph_data), obstacles_(obstacles), Config_(&cfg)
{
}
                                         ...
                                         SpeedData *const speed_data)
{
  ...
  {
    ROS_INFO("[%s]:Failed to search graph with dynamic programming.", Config_->role_name_.c_str());
    return false;
  }
  return true;
}
...
{
  GriddedPathTimeGraph st_graph(st_graph_data_, obstacles_, init_point_, *Config_);
