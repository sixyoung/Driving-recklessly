...
QpPiecewiseStGraph::QpPiecewiseStGraph(const Param_Configs &Cfg)
    : Config_(&Cfg), t_evaluated_resolution_(Config_->total_time / Config_->number_of_evaluated_graph_t)
{
    Init();
}
    ...
    const std::pair<double, double> &accel_bound)
{
    ...
    generator_.reset(new PiecewiseLinearGenerator(
        ...
        t_evaluated_resolution_, *Config_));
...
bool QpPiecewiseStGraph::AddCruiseReferenceLineKernel(
    const planner::SpeedLimit &speed_limit, const double weight)
{
