...
void QpSplineStGraph::Init()
{
    ...
    {
        t_knots_.push_back(curr_t); // 0，2.0， 4.0，6.0
    ...
    curr_t = 0.0;
    t_evaluated_resolution_ = Config_->total_time / (num_evaluated_t - 1); // 8/30
                             ...
                             SpeedData *const speed_data)
{
    ...
    if (Config_->change_speed && Config_->default_cruise_speed<=0.5)
    {
        speed_data->clear();
    ...
    for (const auto &boundary : st_graph_data.st_boundaries())
    {
        if (boundary.IsPointInBoundary({0.0, 0.0}) ||
            (std::fabs(boundary.min_t_) < kBounadryEpsilon &&std::fabs(boundary.min_s_) < 3.0))
        {
            speed_data->clear();
            const double t_output_resolution = Config_->FLAGS_trajectory_time_min_interval;
            double time = 0.0;
...
bool QpSplineStGraph::AddConstraint(
    const TrajectoryPoint &init_point, const planner::SpeedLimit &speed_limit,
    ...
    const std::pair<double, double> &accel_bound)
{
    ...
    */
    ...
    if(Config_->role_name_ != "hero100" && 0){
        planner::FileExporter::ExportStConstraints(
            t_evaluated_,
            s_upper_bound,
            s_lower_bound,
            "/home/bob/文档/备份/demo05/src/test/txt/st_constraints/st_constraints.txt",
            Config_->frame_id,
            true,   // append_mode
            true,   // split_mode
            300     // frames_per_file
...
bool QpSplineStGraph::AddKernel(
    const std::vector<ST_Boundary> &boundaries,
    const planner::SpeedLimit &speed_limit)
{
    ...
    // init point jerk continuous kernel起点处的jerk连续性约束
    ...
    double *const s_lower_bound) const
{
    *s_upper_bound = total_path_s;
    ...
    for (const auto &boundary : boundaries)
    {
        ...
        else if (boundary.boundary_type() == ST_Boundary::BoundaryType::OVERTAKE)
        {
