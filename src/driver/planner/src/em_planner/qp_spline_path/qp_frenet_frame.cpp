...
QpFrenetFrame::QpFrenetFrame(const ReferenceLine &reference_line, const SpeedData &speed_data,
                             ...
                             const std::vector<double> &evaluated_s, const Param_Configs &cfg)
...
bool QpFrenetFrame::MapStaticObstacleWithDecision(const Obstacle &obstacle)
{
  ...
  if (!MapNudgePolygon(math::Polygon2d(obstacle.PerceptionBoundingBox()), decision.nudge(),
...
*/
bool QpFrenetFrame::MapNudgePolygon(const math::Polygon2d &polygon, const ObjectNudge &nudge,
                                                              ...
                                                              const double s_end)
{
  ...
  SLPoint front = math::InterpolateUsingLinearApproximation(start, end, weight_front);
  SLPoint back = math::InterpolateUsingLinearApproximation(start, end, weight_back);
