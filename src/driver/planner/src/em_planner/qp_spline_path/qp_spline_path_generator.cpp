...
QpSplinePathGenerator::QpSplinePathGenerator(Spline1dSolver *spline_solver, const ReferenceLine &reference_line, const Param_Configs &cfg)
    : spline_solver_(spline_solver), reference_line_(reference_line), Config_(&cfg)
{
}
                                     ...
                                     bool is_final_attempt, PathData *const path_data)
{
  ...
  if (!path_data_history.empty())
  {
    ...
    last_frennet_path_ = &path_data_history.back().second;
  }
  ...
  if (is_change_lane_path_)
  {
    ...
    ref_l_ = 0; //目标车道中心线作为参考线

  }
  double start_s = init_frenet_point_.s;

  double kDefaultPathLength = Config_->PathLength;
  double end_s = std::fmin(init_frenet_point_.s + std::fmax(kDefaultPathLength, init_trajectory_point_.v * Config_->look_forward_time_sec),
  ...
  {
    std::cout << "Init smoothing spline failed with (start_s:" << start_s << ",  end_s " << end_s << ")" << std::endl;
    return false;
  }

  QpFrenetFrame qp_frenet_frame(reference_line_, speed_data, init_frenet_point_, Config_->time_resolution, evaluated_s_, *Config_);
...
bool QpSplinePathGenerator::InitSpline(const double start_s, const double end_s)
{
  ...
  math::uniform_slice(start_s, end_s, number_of_spline, &knots_);
  ...
  constraint_num = std::max(2u, constraint_num); //添加!!!!!!!!!!!!!!!!!
  math::uniform_slice(start_s, end_s, constraint_num - 1, &evaluated_s_);
...
bool QpSplinePathGenerator::AddConstraint(const QpFrenetFrame &qp_frenet_frame, const double boundary_extension)
{
  ...
  // 限定了min_d' <= d'(s) <= max_d'
  spline_constraint->AddPointDerivativeConstraintInRange(init_frenet_point_.s, init_frenet_point_.d_d, kBoundaryEpsilon);
  ...
  if (init_trajectory_point_.v > Config_->uturn_speed_limit)
  {
    ...
    // 限定了min_d'' <= d''(s) <= max_d''
    spline_constraint->AddPointSecondDerivativeConstraintInRange(init_frenet_point_.s, init_frenet_point_.d_dd, kBoundaryEpsilon);
  ...
  for (uint32_t i = 0; i < evaluated_s_.size(); ++i)
  {
    ...
    if(is_change_lane_path_){
      boundary_low.emplace_back(
          math::MaxElement(std::vector<double>{-8, static_obs_boundary.first}));
      boundary_high.emplace_back(
          math::MinElement(std::vector<double>{8, static_obs_boundary.second}));
    }else{
      boundary_low.emplace_back(
          math::MaxElement(std::vector<double>{road_boundary.first, static_obs_boundary.first}));
      boundary_high.emplace_back(
          math::MinElement(std::vector<double>{road_boundary.second, static_obs_boundary.second}));
    }
  ...
  if(Config_->role_name_ == "hero0" && 0){
    planner::FileExporter::ExportLateralBoundaries(
        evaluated_s_,
        boundary_low,
        boundary_high,
        "/home/bob/文档/备份/demo05/src/test/txt/lateral_bounds_log/lateral_bounds_log.txt",
        Config_->frame_id,
        true,   
        true,   
        300);
...
void QpSplinePathGenerator::AddHistoryPathKernel()
{
  ...
  // 旧版本：PathData last_path_data(reference_line_);
