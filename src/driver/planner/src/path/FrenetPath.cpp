...
FrenetFramePoint FrenetFramePath::EvaluateByS(const double s) const
{
  ...
  FrenetFramePoint p;
  p.set_s(s);
  p.set_l(math::lerp(p0.d, s0, p1.d, s1, s));
  p.set_dl(math::lerp(p0.d_d, s0, p1.d_d, s1, s));
  p.set_ddl(math::lerp(p0.d_dd, s0, p1.d_dd, s1, s));
  return p;
}
...
PathPoint DiscretizedPath::EvaluateReverse(const double path_s) const
{
  ...
  return math::InterpolateUsingLinearApproximation(*(it_upper - 1), *it_upper, path_s);
}
