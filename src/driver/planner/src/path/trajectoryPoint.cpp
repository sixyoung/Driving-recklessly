...
size_t DiscretizedTrajectory::QueryForwardNearestPoint(
    const Vec2d &position, double yaw) const
{
    double dist_sqr_min = std::numeric_limits<double>::max();
    size_t index_min = 0;

    // 车辆朝向向量
    Vec2d heading_vec(std::cos(yaw), std::sin(yaw));
    ...
    for (size_t i = 0; i < size(); ++i)
    {
        ...
        // 方向判断：点是否在前方
        Vec2d vec_to_point(curr_point.x() - position.x(),
                           curr_point.y() - position.y());
        if (heading_vec.InnerProd(vec_to_point) < 0)
        {
            continue; // 在后方 → 跳过
        }
