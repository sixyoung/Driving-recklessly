...
ST_Boundary::ST_Boundary(const std::vector<std::pair<STPoint, STPoint>> &point_pairs, bool is_accurate_boundary, const Param_Configs& cfg)
    : obstacle_id(obstacle_id), Config_(&cfg)
{
...
bool ST_Boundary::IsPointInBoundary(const STPoint &st_point) const
{
  ...
  const double check_upper = math::CrossProd(
      st_point, upper_points_[left], upper_points_[right]);
  const double check_lower = math::CrossProd(
...
void ST_Boundary::finalize_polygon() {
    std::vector<Vec2d> poly_points;
    poly_points.emplace_back(bottom_left_point_.t(), bottom_left_point_.s());
    poly_points.emplace_back(upper_left_point_.t(),  upper_left_point_.s());
    poly_points.emplace_back(upper_right_point_.t(), upper_right_point_.s());
    poly_points.emplace_back(bottom_right_point_.t(), bottom_right_point_.s());

    // 直接调用 Polygon2d 的构造函数重新赋值
    *static_cast<Polygon2d*>(this) = Polygon2d(poly_points);
}