...
ObstacleTest::ObstacleTest(bool is_use_carla, const Param_Configs& cfg) : Config_(&cfg), oba(*Config_)
{
  ros::NodeHandle nh_;
...
{
  // std::cout << "x_1:" << x_pose_covariance << "\n";
  // std::cout << "x_2:" << x_pose_covariance1 << "\n";
  // 求障碍物顶点,往障碍物的运动方向看
  PPoint left_front;
  PPoint left_buttom;
  PPoint right_front;
...
void ObstacleTest::Publish_StaticObstacle()
{
  ...
  // Object.shape.footprint.points[0].x = 24.3014;
  // Object.shape.footprint.points[0].y = -28.0064; 
  // Object.shape.footprint.points[1].x = 22.7826;
  // Object.shape.footprint.points[1].y = -30.0477;
  // Object.shape.footprint.points[2].x = 19.2515;
  // Object.shape.footprint.points[2].y = -27.0503;
  // Object.shape.footprint.points[3].x = 21.196;
  // Object.shape.footprint.points[3].y = -25.4315;
...
void ObstacleTest::thread_obstacle_test(void)
{
  ...
  caculate_points(obstacle_test_path + "/sim_overtake/", sample_x1, sample_y1, sample_head1);
  caculate_points(obstacle_test_path + "/sim_follow_vehicle/", sample_x2, sample_y2, sample_head2);
  ...
  while (n4.ok())
  {
    ...
    // //运动障碍物
    Publish_DynamicObstacle(sample_x1[i], sample_y1[i], sample_head1[i], sample_x2[j], sample_y2[j], sample_head2[j]);
    sleep(1.0);
    j++;
    i++;
    // std::cout << "i:" << i << "\n";
    ...
    //  std::cout << "Object_x:" << Objects.objects[0].state.pose_covariance.pose.position.x<< "\n";
    ros::spinOnce();
    loop_rate.sleep();
  }
}
