...
bool SpeedData::EvaluateByTime(const double t, SpeedPoint *const speed_point) const
{
  ...
  else
  {
    ...
    speed_point->Clear();
    speed_point->set_s(math::lerp(p0.s, t0, p1.s, t1, t));
    speed_point->set_t(t);
    ...
    if (p0.has_v() && p1.has_v())
    {
      // std::cout << "has_v"
      //           << "\n";
      speed_point->set_v(math::lerp(p0.v, t0, p1.v, t1, t));
    }
    if (p0.has_a() && p1.has_a())
    {
      // std::cout << "has_a"
      //           << "\n";
      speed_point->set_a(math::lerp(p0.a, t0, p1.a, t1, t));
    }
    if (p0.has_da() && p1.has_da())
    {
      // std::cout << "has_da"
      //           << "\n";
      speed_point->set_da(math::lerp(p0.da, t0, p1.da, t1, t));
    }
  }
  return true;
}
...
bool SpeedData::EvaluateByS(const double s, SpeedPoint *const speed_point) const
{
  ...
  else
  {
    ...
    speed_point->Clear();
    speed_point->set_s(s);
    speed_point->set_t(math::lerp(p0.t, s0, p1.t, s1, s));
    if (p0.has_v() && p1.has_v())
    {
      speed_point->set_v(math::lerp(p0.v, s0, p1.v, s1, s));
    }
    if (p0.has_a() && p1.has_a())
    {
      speed_point->set_a(math::lerp(p0.a, s0, p1.a, s1, s));
    }
    if (p0.has_da() && p1.has_da())
    {
      speed_point->set_da(math::lerp(p0.da, s0, p1.da, s1, s));
    }
  }
  return true;
}
