...
bool LineSegment2d::IsPointIn(const Vec2d& point) const
{
  ...
  const double prod = math::CrossProd(point, start_, end_);
...
bool LineSegment2d::GetIntersect(const LineSegment2d& other_segment, Vec2d* const point) const
{
  ...
  const double cc1 = math::CrossProd(start_, end_, other_segment.start());
  const double cc2 = math::CrossProd(start_, end_, other_segment.end());
  if (cc1 * cc2 >= -kMathEpsilon)
  {
    return false;
  }
  const double cc3 = math::CrossProd(other_segment.start(), other_segment.end(), start_);
  const double cc4 = math::CrossProd(other_segment.start(), other_segment.end(), end_);
