...
bool Polygon2d::IsPointIn(const Vec2d& point) const
{
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    ...
    {
      const double side = math::CrossProd(point, points_[i], points_[j]);
...
void Polygon2d::BuildFromPoints()
{
  ...
  {
    area_ += math::CrossProd(points_[0], points_[i - 1], points_[i]);
  }
  if (area_ < 0)
  {
    area_ = -area_;
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    if (math::CrossProd(points_[Prev(i)], points_[i], points_[Next(i)]) <= -kMathEpsilon)
    {
      is_convex_ = false;
      break;
    }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d>& points, Polygon2d* const polygon)
{
  ...
  for (int i = 0; i < n + n; ++i)
  {
    ...
    while (count > last_count &&
           math::CrossProd(points[results[count - 2]], points[results[count - 1]], pt) <= kMathEpsilon)
    {
      results.pop_back();
      --count;
    }
...
bool Polygon2d::ClipConvexHull(const LineSegment2d& line_segment, std::vector<Vec2d>* const points)
{
  ...
  {
    prod[i] = math::CrossProd(line_segment.start(), line_segment.end(), (*points)[i]);
