...
SpeedLimitDecider::SpeedLimitDecider(const SL_Boundary &adc_sl_boundary,
                                     ...
                                     const PathData &path_data, const Param_Configs& cfg)
...
void SpeedLimitDecider::GetSpeedLimits(
    ...
    planner::SpeedLimit *const speed_limit_data) const
{
    ...
    for (uint32_t i = 0; i < discretized_path.size(); ++i)
    {
        ...
        if (i + 1 < discretized_path.size())
        {
            ...
            centri_jerk_speed_limit = std::fmax(
                30.0, Config_->centri_jerk_speed_coeff / centri_jerk); //10
        }
        ...
        if (Config_->FLAGS_enable_nudge_slowdown)
        {
            curr_speed_limit =
                std::fmax(Config_->lowest_speed,
                          math::MinElement(std::vector<double>{
        ...
        if(Config_->merge_onramp_over_speed){
            curr_speed_limit =
                std::fmax(Config_->lowest_speed,
                          math::MinElement(std::vector<double>{
                              speed_limit_on_reference_line, 30 ,
                              30}));
        }
