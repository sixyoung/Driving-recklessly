...
namespace planner
{
  namespace math
  {
    ...
    PathPoint InterpolateUsingLinearApproximation(const PathPoint &p0, const PathPoint &p1, const double s)
    {
      ...
      double theta = math::slerp(p0.theta, p0.s, p1.theta, p1.s, s);
      double kappa = (1 - weight) * p0.kappa + weight * p1.kappa;
      double dkappa = (1 - weight) * p0.dkappa + weight * p1.dkappa;
      // 🔔 检查 theta
      // if (theta < -1.0) {
      //   ROS_WARN_STREAM("[InterpolateUsingLinearApproximation] theta=" << theta
      //                   << " < -1 at s=" << s
      //                   << ", between s0=" << s0 << " (theta0=" << p0.theta
      //                   << ") and s1=" << s1 << " (theta1=" << p1.theta << ")");
      // }
      path_point.set_x(x);
      path_point.set_y(y);
                                                        ...
                                                        const double t)
    {
      ...
      TrajectoryPoint tp;
      tp.set_v(math::lerp(tp0.v, t0, tp1.v, t1, t));
      tp.set_a(math::lerp(tp0.a, t0, tp1.a, t1, t));
      tp.set_relative_time(t);
      tp.set_x(math::lerp(pp0.x, t0, pp1.x, t1, t));
      tp.set_y(math::lerp(pp0.y, t0, pp1.y, t1, t));
      tp.set_theta(math::slerp(pp0.theta, t0, pp1.theta, t1, t));
      tp.set_kappa(math::lerp(pp0.kappa, t0, pp1.kappa, t1, t));
      tp.set_dkappa(math::lerp(pp0.dkappa, t0, pp1.dkappa, t1, t));
      tp.set_s(math::lerp(pp0.s, t0, pp1.s, t1, t));
