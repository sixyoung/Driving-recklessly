...
public:
  ...
  {
    if (boundary_type_ == BoundaryType::UNKNOWN)
    {
      return "UNKNOWN";
    }
    else if (boundary_type_ == BoundaryType::CAR)
    {
      return "CAR";
    }
    else if (boundary_type_ == BoundaryType::TRUCK)
    {
      return "TRUCK";
    }
    else if (boundary_type_ == BoundaryType::BUS)
    {
      return "BUS";
    }
    else if (boundary_type_ == BoundaryType::BICYCLE)
    {
      return "BICYCLE";
    }
    else if (boundary_type_ == BoundaryType::MOTORBIKE)
    {
      return "MOTORBIKE";
    }
      else if (boundary_type_ == BoundaryType::PEDESTRIAN)
    {
      return "PEDESTRIAN";
    }    else if (boundary_type_ == BoundaryType::ANIMAL)
    {
      return "ANIMAL";
    }    else if (boundary_type_ == BoundaryType::SCHEDULE_BEFORE)
    {
      return "SCHEDULE_BEFORE";
    }    else if (boundary_type_ == BoundaryType::SCHEDULE_AFTER)
    {
...
private:
  BoundaryType boundary_type_ = BoundaryType::UNKNOWN; //障碍物边界类型
  std::vector<SLPoint> boundary_points;

public:
  std::string obstacle_id = "not_set";
  //障碍物左右位置
...
class ST_Boundary : public math::Polygon2d
{
public:
  ST_Boundary(const Param_Configs& cfg): Config_(&cfg) {}
  ST_Boundary(const std::vector<std::pair<STPoint, STPoint>> &point_pairs, bool is_accurate_boundary, const Param_Configs& cfg);
  ~ST_Boundary() = default;
  ...
  std::vector<STPoint> getLower_points() const
  {
