...
{
public:
  StGraphData(const Param_Configs &cfg): Config_(&cfg) {};

  void LoadData(const std::vector<ST_Boundary> &st_boundaries, const double min_s_on_st_boundaries,
                const TrajectoryPoint &init_point, const planner::SpeedLimit &speed_limit, const double cruise_speed,
  ...
  const planner::SpeedLimit &speed_limit() const;
...
private:
  ...
  planner::SpeedLimit speed_limit_;
