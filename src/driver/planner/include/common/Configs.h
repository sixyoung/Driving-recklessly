...
public:
  YAML::Node config;
  // cost
  mutable std::string role_name_; 
  mutable std::string scene_type; 
  mutable size_t frame_id;
  mutable bool avoid_vehicle_;
  mutable bool avoid_traffic_light_;
  mutable bool lane_change;
  mutable bool sudden_speed_change;
  mutable bool change_speed;
  mutable bool recovery_speed;
  mutable bool speed_abnormal;
  mutable bool use_recovery_by_point;
  mutable bool merge_onramp_over_speed;

  mutable double lane_change_cruise_speed;
  mutable double lane_change_planning_upper_speed_limit;
  mutable double last_cruise_speed;
  mutable double last_planning_upper_speed_limit;

  mutable double speed_change_cruise_speed;
  mutable double speed_change_planning_upper_speed_limit;
  mutable double recovery_start_time;
  mutable double recovery_wait_time;

  mutable bool is_avoid_vehicle_;
  mutable bool elapsed_time;
  mutable bool abnormal;
  mutable std::vector<std::string> ignored_vehicle_roles;

  mutable double detection_range;
  ...
  mutable double default_cruise_speed; // 巡航速度
  double default_init_speed;
  double dynamic_obs_predict_time;
  mutable double planning_upper_speed_limit; // Maximum speed (m/s) in planning.
  ...
  double unit_t;
  mutable double safe_distance;
  mutable double overtake_distance_s;
  ...
  mutable bool IsChangeLanePath;            // 如果可以变道
  ...
  double FLAGS_replan_lateral_distance_threshold;
  mutable double FLAGS_replan_longitudinal_distance_threshold;
  // QpStSpeedConfig
  ...
  mutable double high_speed_centric_acceleration_limit;
  mutable double low_speed_centric_acceleration_limit;
  mutable double high_speed_threshold;
  mutable double low_speed_threshold;
  ...
  double FLAGS_min_stop_distance_obstacle;
...
public:
  ...
  bool SetParamValue(const std::string& name, double value);
  bool SetParamValue(const std::string& name, int value);
  bool SetParamValue(const std::string& name, bool value);
  bool SetParamValue(const std::string& name, size_t value);
  bool SetParamValue(const std::string& name,const std::vector<std::string>& values);
  bool SetParamValueAuto(const std::string& name, double value);
public:
  ...
  std::unordered_map<std::string, uint32_t*> uint_params_;
  std::unordered_map<std::string, std::string*> string_params_;  // ← 新增
  std::unordered_map<std::string, size_t*> size_t_params_;       // ← 新增
  std::unordered_map<std::string, std::vector<std::string>*> list_params_;       // ← 新增
};
